...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):        
        ...
        if True:#self.opt.label_nc == 0:
            ...
            A_tensor = transform_A(A.convert('RGB'))[:1]  #<<TODO:move rgb out
        # else:
        # if True:
        #     transform_A = get_transform(self.opt, params, method=Image.NEAREST, normalize=False)
        #     A_tensor = transform_A(A)

        B_tensor = C_tensor = 0# = inst_tensor = feat_tensor = 0
        ### input B (real images)
        ...
        input_dict = {'label': A_tensor, 'image': B_tensor, 'edge': C_tensor, 'path': A_path}
