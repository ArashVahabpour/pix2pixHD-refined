...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            netD1_input_nc = input_nc + netG1_output_nc
            netD2_input_nc = input_nc + netG2_output_nc
            # if not opt.no_instance:
            #     netD_input_nc += 1
    ...
    def forward(self, label, image, edge, infer=False):
        ...
        pred_fake_pool1 = self.discriminate(input_label, fake_image_and_edge1, netD_idx=1, use_pool=True)
        pred_fake_pool2 = self.discriminate(input_label, fake_image_and_edge2, netD_idx=2, use_pool=True)
        ...
        pred_fake1 = self.netD1.forward(torch.cat((input_label, fake_image_and_edge1), dim=1))
        pred_fake2 = self.netD2.forward(torch.cat((input_label, fake_image_and_edge2), dim=1))
