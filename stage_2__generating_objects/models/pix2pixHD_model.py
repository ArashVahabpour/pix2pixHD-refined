...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            netD2_input_nc = netG2_input_nc + opt.output_nc2
            # if not opt.no_instance:
            #     netD_input_nc += 1
    ...
    def encode_input(self, label_map, inst_map=None, real_edge=None, real_image=None, feat_map=None, infer=False):
    ...
    def forward(self, label, inst, edge, image, feat, infer=False):
        # Encode Inputs
        input_label, inst_map, real_edge, real_image, feat_map = self.encode_input(label, inst, edge, image, feat)
        ...
        fake_image1 = self.netG1.forward(input_concat)
        fake_image2 = self.netG2.forward(fake_image1)

        # Fake Detection and Loss
        pred_fake_pool1 = self.discriminate(input_label, fake_image1, netD_idx=1, use_pool=True)
        # pred_fake_pool2 = self.discriminate(fake_image1, fake_image2, netD_idx=2, use_pool=True)
        loss_D_fake = self.criterionGAN(pred_fake_pool1, False) #+ self.criterionGAN(pred_fake_pool2, False)

        # Real Detection and Loss        
        pred_real1 = self.discriminate(input_label, real_edge, netD_idx=1)
        # pred_real2 = self.discriminate(input_label, real_image, netD_idx=2)
        loss_D_real = self.criterionGAN(pred_real1, True) #+ self.criterionGAN(pred_real2, True)

        # GAN loss (Fake Passability Loss)        
        pred_fake1 = self.netD1.forward(torch.cat((input_label, fake_image1), dim=1))
        # pred_fake2 = self.netD2.forward(torch.cat((fake_image1, fake_image2), dim=1))
        ...
        loss_G_VGG = 0
        loss_G_VGG += self.criterionL1(fake_image1, real_edge)
        # if not self.opt.no_vgg_loss:
        #     loss_G_VGG = self.criterionVGG(fake_image2.repeat(1,3,1,1), real_image.repeat(1,3,1,1)) * self.opt.lambda_feat
        
        # Only return the fake_B image if necessary to save BW
        return [ self.loss_filter( loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_D_real, loss_D_fake ), None if not infer else (fake_image1, fake_image2) ]

    def inference(self, label, inst, edge=None, image=None):
        # Encode Inputs
        image = Variable(image) if image is not None else None
        input_label, inst_map, real_edge, real_image, _ = self.encode_input(Variable(label), Variable(inst), edge, image, infer=True)
        ...
        if torch.__version__.startswith('0.4'):
            ...
            fake_image1 = self.netG1.forward(input_concat)
            fake_image2 = self.netG2.forward(fake_image1)
        return fake_image1, fake_image2
