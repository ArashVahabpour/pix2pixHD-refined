...
def resize_to_inception(input_image, w, h, interpolation):
    """
    Args:
        input_image: image to be resized to inception size

    Output:
        a 299x299 resized image
    """
    ratio = 299. / max(w, h)
    output_image = cv.resize(input_image, (299, 299), interpolation=interpolation)
...
def create_dataset(source_dir, destination_dir, grayscale=False, separate_biggest=True, separate_two_biggest=True,inception=False):
    ...
    for subset in ['train', 'test']:#'train', 'test':
        ...
        for image_filename in glob.glob(source_dir + subset + '_img/*.png'):
            ...
            for i, (obj_mask, mask_type) in enumerate(zip(obj_mask_list + obj_mask_biggest_list + obj_mask_two_biggest_list,
                ...
                if w == mx:
                    im =  image[y1 + h//2 - w//2:y1 + h// 2 + w//2, x1:x1 + w]
                    if im.shape[0]==0 or im.shape[1]==0:
                        im = image[y1:y1+h, x1:x1+w]
                        if im.shape[0] * ratio < 1 or im.shape[1] * ratio < 1:
                            continue
                else:
                    im =  image[y1 :y1 + h, x1 + w//2 - h//2:x1 + w//2 + h//2]
                    if im.shape[0]==0 or im.shape[1]==0:
                        im = image[y1:y1+h, x1:x1+w]
                        if im.shape[0] * ratio < 1 or im.shape[1] * ratio < 1:
                            continue
                #     if y1-w//2 >0 and y1+w//2<image.shape[0]:
                #         im = image[y1-w//2:y1+h+w//2,x1:x1+w]
                #     else:# y1-w//2 >0 and not y1+w//2<image.shape[0]:
                #         im = image[y1:y1+w,x1:x1+w]
                # else:
                #     if x1-h//2 >0 and x1+h//2<image.shape[1]:
                #         im = image[y1:y1+h,x1-h//2:x1+h//2]
                #     else:# y1-w//2 >0 and not y1+w//2<image.shape[0]:
                #         im = image[y1:y1+h,x1:x1+h]
                ...
                obj = resize_to_inception(im,im.shape[0],im.shape[1] , cv.INTER_LINEAR)
...
def main():
    create_dataset(source_dir='/home/shared/datasets/cityscapes.pix2pixHD.folders/',
                   destination_dir='/home/shared/datasets/carstest/')
